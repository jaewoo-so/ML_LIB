...
def Test_Classification_TestFold(verbose = True):
    ...
    for name in name_list:
        ...
        auc_score_list[name] = roc_auc_score(np.where(y > y.mean() , 1 ,0 ) , np.argmax(fold_oof.mean(axis = 0) , axis = 1))
...
#Test_Classification() # test done
#est_Classification_TestFold(False) # test done
